...
export function camelToSnake(
  ...
  if (Array.isArray(obj)) {
    return obj.map(item => camelToSnake(item as Record<string, unknown>)) as Record<
      string,
      unknown
    >[];
  }
...
export function snakeToCamel(
  ...
  if (Array.isArray(obj)) {
    return obj.map(item => snakeToCamel(item as Record<string, unknown>)) as Record<
      string,
      unknown
    >[];
  }
...
export const numericSchema = z
  .union([z.number(), z.string().regex(/^\d+(\.\d+)?$/, 'Must be a valid number')])
...
export const fieldValidators = {
  ...
  requiredUuid: (fieldName: string) => uuidSchema.describe(`${fieldName} (UUID required)`),

  optionalUuid: (fieldName: string) => optionalUuidSchema.describe(`${fieldName} (UUID, optional)`),

  nullableUuid: (fieldName: string) => nullableUuidSchema.describe(`${fieldName} (UUID, nullable)`),
  ...
  workOrderStatus: createEnumSchema(
  ...
  userRole: createEnumSchema(
    ['technician', 'supervisor', 'manager', 'admin', 'inventory_clerk', 'contractor', 'requester'],
    'User Role'
  ),
