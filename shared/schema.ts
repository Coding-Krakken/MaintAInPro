...
export const workOrders = pgTable('work_orders', {
  ...
  status: text('status')
    .notNull()
    .$type<'new' | 'assigned' | 'in_progress' | 'completed' | 'verified' | 'closed'>(),
...
export const escalationRules = pgTable('escalation_rules', {
  ...
  escalationAction: text('escalation_action')
    .notNull()
    .$type<'notify_supervisor' | 'notify_manager' | 'auto_reassign'>(),
...
export const jobQueue = pgTable('job_queue', {
  ...
  jobType: text('job_type')
    .notNull()
    .$type<'escalation_check' | 'pm_generation' | 'notification_send'>(),
...
export const activityLogs = pgTable('activity_logs', {
  ...
  severity: text('severity')
    .default('info')
    .$type<'debug' | 'info' | 'warn' | 'error' | 'critical'>(),
