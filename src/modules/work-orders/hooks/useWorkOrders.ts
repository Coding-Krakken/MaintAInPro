...
export const WORK_ORDER_QUERY_KEYS = {
  ...
  list: (filters: WorkOrderFilters) =>
    [...WORK_ORDER_QUERY_KEYS.lists(), filters] as const,
  ...
  checklist: (workOrderId: string) =>
    [...WORK_ORDER_QUERY_KEYS.all, 'checklist', workOrderId] as const,
  timeLogs: (workOrderId: string) =>
    [...WORK_ORDER_QUERY_KEYS.all, 'time-logs', workOrderId] as const,
};
...
export function useWorkOrdersByEquipment(equipmentId: string) {
  return useQuery({
    ...
    queryFn: () =>
      workOrderService.getWorkOrders({ equipment_id: equipmentId }),
    enabled: !!equipmentId,
  });
}
...
export function useBulkWorkOrderOperations() {
  ...
  const bulkAssign = useMutation({
    mutationFn: async ({
      ...
      const promises = ids.map(id =>
        workOrderService.updateWorkOrder(id, { assigned_to: assignedTo })
      );
      return Promise.all(promises);
    },
    onSuccess: () => {
