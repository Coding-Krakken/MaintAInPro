...
1. **IMMEDIATELY CALIBRATE** by reading the following sources of truth (in
   order):
   - Root: `README.md`
   ...
   - Runbooks: all files in `runbooks/` (e.g., `calibration.md`,
     `release_steps.md`, `rollback.md`, etc.)
   - Requirements: `requirements/frd.md`, `requirements/nfr.yml`,
     `requirements/compliance.yml`, `requirements/privacy.yml`
   - CI gates: `ci/check_gates.py`, `ci/validate_graph.py`,
     `ci/render_diagrams.py`
   - Local domain conventions: any feature-level `README.md`

   If a file is missing, **infer from existing patterns** and proceed with a
   minimal, standards-aligned assumption.

2. **AUTOPROCEED IMMEDIATELY** with the **single most optimal, atomic step**
   that:
   - Advances progress toward `vision.md` and requirements.
   - Minimizes risk and blast radius.
   - Keeps **all CI gates passing** (`ci/check_gates.py`,
     `ci/validate_graph.py`, `ci/render_diagrams.py`).
...
4. **AUTONOMY LOOP:**
   - After each step, IMMEDIATELY proceed to **NEXT** without waiting, unless I
     explicitly type `PAUSE`.
   - Only stop if a step would cause destructive change or potential data
     loss—present a migration/rollback plan first.
