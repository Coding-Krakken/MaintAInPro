...
const RealTimePartsConsumption: React.FC<{ workOrderId?: string }> = ({ workOrderId }) => {
  ...
  const { data: partsUsage = [] } = useQuery<PartsUsage[]>({
    ...
    queryFn: () =>
      workOrderId
        ? fetch(`/api/work-orders/${workOrderId}/parts-usage`).then(res => res.json())
        : Promise.resolve([]),
    enabled: !!workOrderId,
  });
  ...
  useEffect(() => {
    ...
    latestNotifications.forEach(update => {
      if (
        ...
        !recentUpdates.some(r => r.partId === update.partId)
      ) {
        toast({
  ...
  return (
    <div className='space-y-6'>
      ...
      <Card
        className={`border-2 ${isConnected ? 'border-green-200 bg-green-50' : 'border-yellow-200 bg-yellow-50'}`}
      >
        <CardContent className='p-4'>
          <div className='flex items-center justify-between'>
            <div className='flex items-center space-x-2'>
              <Activity
                className={`w-5 h-5 ${isConnected ? 'text-green-600' : 'text-yellow-600'}`}
              />
              <span className='font-medium'>
                Real-time Inventory {isConnected ? 'Connected' : 'Disconnected'}
              </span>
            </div>
            <div className='text-sm text-gray-600'>{recentUpdates.length} recent updates</div>
          </div>
        </CardContent>
      </Card>
            ...
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-2xl font-bold text-red-600'>
                  {
                    parts.filter(p => p.status === 'low_stock' || p.status === 'out_of_stock')
                      .length
                  }
                </p>
              </div>
                  ...
                  <div className='space-y-4'>
                          ...
                          <CardContent className='p-4'>
                            <div className='flex items-center justify-between'>
                              ...
                              <div className='text-right space-y-1'>
                                ...
                                <div className='text-lg font-semibold'>{part.stockLevel} units</div>
                            ...
                            <div className='flex items-center space-x-3'>
                              ...
                              <div className='flex items-center space-x-2'>
                                <Button
                                  ...
                                  size='sm'
                                  onClick={() =>
                                    setConsumptionQuantity(Math.max(1, consumptionQuantity - 1))
                                  }
                                >
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    setConsumptionQuantity(
                                      Math.min(selectedPart.stockLevel, consumptionQuantity + 1)
                                    )
                                  }
                            ...
                            <div className='mt-3 p-3 bg-red-100 border border-red-300 rounded-lg'>
                              <div className='flex items-center space-x-2 text-red-700'>
                                ...
                                <span className='text-sm'>
                                  Insufficient stock! Available: {selectedPart.stockLevel} units
                                </span>
                              </div>
                            </div>
                          )}
                    ...
                    <div className='flex justify-end space-x-4 pt-4 border-t'>
                      ...
                      <Button
                        ...
                        disabled={
                          !selectedPart ||
                          consumptionQuantity > (selectedPart?.stockLevel || 0) ||
                ...
                <Card key={usage.id} className='border-l-4 border-l-blue-500'>
                  <CardContent className='p-4'>
                    <div className='flex items-center justify-between'>
                      ...
                      <div className='text-right'>
                        <div className='font-semibold text-lg'>${usage.totalCost.toFixed(2)}</div>
