    ...
    <div>
      ...
      <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8'>
        <div>
          ...
          <p className='text-gray-600 mt-1'>Manage parts and inventory levels</p>
        </div>
            ...
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-3xl font-bold text-gray-900'>
                  ...
                  {parts
                    ?.reduce(
                      (sum, part) => sum + parseFloat(part.unitCost || '0') * part.stockLevel,
                      0
                    )
                    .toFixed(2) || '0.00'}
                </p>
                  ...
                  {filteredParts.map(part => {
                    ...
                    return (
                      <tr key={part.id} className='border-b hover:bg-gray-50'>
                        <td className='py-3 px-4 font-medium text-gray-900'>{part.partNumber}</td>
                        <td className='py-3 px-4 text-gray-700'>{part.description}</td>
                        <td className='py-3 px-4 text-gray-700'>{part.category || '-'}</td>
                        <td className='py-3 px-4'>
                          <div className='flex items-center space-x-2'>
                            <span className='font-medium'>{part.stockLevel}</span>
                            <span className='text-gray-500 text-sm'>/ {part.reorderPoint} min</span>
                          </div>
                        </td>
                        <td className='py-3 px-4'>
                        ...
                        <td className='py-3 px-4 text-gray-700'>{part.location || '-'}</td>
                      </tr>
                    );
                  })}
                </tbody>
      ...
      {/* Add Part Modal */}
      <PartFormModal isOpen={isPartModalOpen} onClose={() => setIsPartModalOpen(false)} />
    </div>
  );
}
