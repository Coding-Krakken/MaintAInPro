...
function calculateMTTR(workOrders: any[]): number {
  ...
  const correctiveOrders = workOrders.filter(wo => 
    wo.type === 'corrective' && ['completed', 'verified', 'closed'].includes(wo.status) && wo.completedAt
  );
...
router.get('/work-orders', 
  validateSchema(z.object({
    ...
    status: z.enum(['new', 'assigned', 'in_progress', 'completed', 'verified', 'closed']).optional(),
  ...
  async (req: any, res) => {
    try {
      ...
      if (filters.equipmentId) {
        filteredWorkOrders = filteredWorkOrders.filter(wo => wo.equipmentId === filters.equipmentId);
      }
  ...
  async (req: any, res) => {
    try {
      ...
      if (!workOrderData.foNumber) {
        ...
        const existingWorkOrders = await storage.getWorkOrders(req.user?.warehouseId || workOrderData.warehouseId);
...
// Create new part
router.post('/parts',
  validateSchema(insertPartSchema),
  async (req: any, res) => {
    try {
...
router.put('/parts/:id',
  ...
  async (req: any, res) => {
    try {
      ...
      if (updateData.stockLevel !== undefined && updateData.stockLevel <= (existingPart.reorderPoint || 0)) {
        try {
          const { notificationService } = await import('../services/notification.service');
          ...
          for (const manager of managers) {
            await notificationService.sendNotification({
              ...
              metadata: { partId: id, currentStock: updateData.stockLevel, reorderPoint: existingPart.reorderPoint }
            });
          }
        } catch (notifError) {
    ...
    try {
      ...
      const analytics = {
        ...
        performance: {
          ...
          completionRate: filteredWorkOrders.length > 0 ? 
            (filteredWorkOrders.filter(wo => ['completed', 'verified', 'closed'].includes(wo.status)).length / filteredWorkOrders.length) * 100 : 0
        }
      };
...
router.patch('/work-orders/bulk/status',
  validateSchema(z.object({
...
router.patch('/work-orders/bulk/assign',
  validateSchema(z.object({
    workOrderIds: z.array(z.string().uuid()).min(1).max(50),
