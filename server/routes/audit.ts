...
export function registerAuditRoutes(
  ...
  // Query audit events
  app.get('/api/audit/events', authenticateRequest, requireRole('admin'), async (req, res) => {
    try {
      const {
        startDate,
        endDate,
      ...
      console.error('Query audit events _error:', _error);
      res.status(500).json({ message: 'Failed to query audit events' });
    }
  });

  // Get audit statistics
  app.get('/api/audit/statistics', authenticateRequest, requireRole('admin'), async (req, res) => {
    try {
      ...
      console.error('Get audit statistics _error:', _error);
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        console.error('Generate compliance report _error:', _error);
  ...
  app.get('/api/audit/events/:id', authenticateRequest, requireRole('admin'), async (req, res) => {
    try {
      const { id } = req.params;
      ...
      console.error('Get audit event _error:', _error);
  ...
  app.get('/api/audit/export', authenticateRequest, requireRole('admin'), async (req, res) => {
    try {
      ...
      console.error('Export audit data _error:', _error);
  ...
  app.get(
    '/api/audit/compliance-dashboard',
    authenticateRequest,
    requireRole('admin'),
    async (req, res) => {
      try {
        ...
        const complianceData = {
          ...
          recommendations: [
            stats.failedEvents > 50
              ? 'High number of failed events detected - review security policies'
              : null,
            stats.eventsByCategory.security > 10
              ? 'Multiple security events - investigate potential threats'
              : null,
            stats.eventsByCategory.authentication > 100
              ? 'Consider implementing stronger authentication controls'
              : null,
          ].filter(Boolean),
        };
        ...
        console.error('Compliance dashboard _error:', _error);
  ...
  app.post('/api/audit/log-event', authenticateRequest, requireRole('admin'), async (req, res) => {
    try {
      ...
      console.error('Manual log event _error:', _error);
