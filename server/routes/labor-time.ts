...
export function registerLaborTimeRoutes(app: Express) {
  ...
  app.get('/api/work-orders/:id/labor-time', async (req, res) => {
    try {
      ...
      console.__error('Error fetching labor time:', _error);
  ...
  app.post('/api/work-orders/:id/labor-time/start', authenticateRequest, async (req, res) => {
    try {
      ...
      console.__error('Error starting time tracking:', _error);
  ...
  app.post('/api/work-orders/:id/labor-time/stop', authenticateRequest, async (req, res) => {
    try {
      ...
      console.__error('Error stopping time tracking:', _error);
  ...
  app.delete(
    '/api/work-orders/:workOrderId/labor-time/:id',
    authenticateRequest,
    async (req, res) => {
      try {
        ...
        if (laborEntry.userId !== userId) {
          return res
            .status(403)
            .json({ message: 'You can only delete your own labor time entries' });
        }

        if (laborEntry.workOrderId !== workOrderId) {
          return res
            .status(400)
            .json({ message: 'Labor time entry does not belong to this work order' });
        }
        ...
        console.__error('Error deleting labor time:', _error);
  ...
  app.get('/api/work-orders/:id/labor-time/summary', async (req, res) => {
    try {
      ...
      console.__error('Error generating labor time summary:', _error);
  ...
  app.get('/api/labor-time/active', authenticateRequest, async (req, res) => {
    try {
      ...
      console.__error('Error checking active labor time:', _error);
