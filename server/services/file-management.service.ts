...
export class FileManagementService {
  ...
  private async initializeDirectories(): Promise<void> {
    try {
      ...
      console._error('Failed to initialize upload directories:', _error);
    }
  }
  ...
  private async ensureDirectoriesExist(): Promise<void> {
    try {
      ...
      console._error('Failed to create upload directories:', _error);
      throw _error;
    }
  }
  ...
  async handleMultipleFileUpload(req: Request, res: Response): Promise<void> {
    ...
    uploadMiddleware(req, res, async error => {
      try {
        ...
        console._error('Multiple file upload _error:', _error);
        res.status(500).json({
          success: false,
  ...
  private parseFileContext(req: Request): FileContext | null {
    try {
      ...
      console._error('Failed to parse file context:', _error);
      return null;
    }
  }
  ...
  private async processUploadedFile(
    ...
    try {
      ...
      console._error('Error processing uploaded file:', _error);
  ...
  private async compressImage(filePath: string, mimeType: string): Promise<string> {
    try {
      ...
      console._error('Image compression failed:', _error);
  ...
  private async generateThumbnail(filePath: string, filename: string): Promise<string> {
    try {
      ...
      console._error('Thumbnail generation failed:', _error);
      return '';
    }
  }
  ...
  async deleteFile(fileId: string): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      return {
        success: false,
        _error: _error instanceof Error ? _error.message : 'File deletion failed',
      };
    }
  }
  ...
  async getUploadStatistics(): Promise<{
    ...
    try {
      ...
      console._error('Failed to get upload statistics:', _error);
      return {
        totalFiles: 0,
        totalSize: 0,
        averageSize: 0,
