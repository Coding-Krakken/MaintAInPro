...
export class SessionService {
  ...
  static async createSession(
    ...
    const duration = options.rememberMe ? this.REMEMBER_ME_DURATION : this.DEFAULT_SESSION_DURATION;
  ...
  private static async enforceSessionLimits(userId: string): Promise<void> {
    ...
    for (let i = 0; i < sessionsToRemove; i++) {
      const sessionObj = sessions[i];
      if (sessionObj) {
        await this.endSession(sessionObj.id);
      }
    }
  }
  ...
  private static async checkSuspiciousActivity(userId: string, ipAddress: string): Promise<void> {
    ...
    const recentSessions = userSessions.filter(
      session =>
        Date.now() - session.lastAccessedAt.getTime() <
        this.SUSPICIOUS_LOGIN_THRESHOLD * 60 * 60 * 1000
    );
