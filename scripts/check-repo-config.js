...
try {
  const labels = JSON.parse(execSync('gh api repos/:owner/:repo/labels', { 
    stdio: 'pipe', 
    encoding: 'utf8' 
  }));
...
try {
  const secrets = JSON.parse(execSync('gh api repos/:owner/:repo/actions/secrets', { 
    stdio: 'pipe', 
    encoding: 'utf8' 
  }));
...
requiredLabels.forEach(label => {
  ...
  console.log(`gh label create "${label}" --color "${colors[label]}" --description "${descriptions[label]}"`);
});
