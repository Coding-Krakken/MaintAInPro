...
const MobileChecklistExecution: React.FC<MobileChecklistExecutionProps> = ({
  ...
  const [localUpdates, setLocalUpdates] = useState<Map<string, Partial<ChecklistItemWithActions>>>(
    new Map()
  );
  ...
  return (
    <div className='w-full max-w-md mx-auto space-y-4'>
      ...
      <div className='flex items-center justify-between p-2 bg-card rounded-lg border'>
        ...
        {pendingCount > 0 && (
          <div className='flex items-center space-x-2'>
            ...
            {networkStatus.isOnline && (
              <Button size='sm' variant='ghost' onClick={() => forceSync()} className='h-6 px-2'>
      ...
      <Card>
        <CardHeader>
          <CardTitle className='text-base font-medium'>{currentItem.component}</CardTitle>
          <p className='text-sm text-muted-foreground'>{currentItem.action}</p>
        </CardHeader>
