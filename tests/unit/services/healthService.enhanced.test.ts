...
describe('HealthService Enhancement Validation', () => {
  it('should export required interfaces and types', async () => {
    ...
    const requiredInterfaces = ['HealthData', 'HealthMetrics', 'HealthService'];

    // Read the health service file content
    const fs = await import('fs/promises');
    const healthServiceContent = await fs.readFile('client/src/services/healthService.ts', 'utf-8');
