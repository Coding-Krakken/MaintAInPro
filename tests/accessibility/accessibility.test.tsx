...
const MockLoginForm = () => (
  <form role='form' aria-label='Login form'>
    ...
    <div id='login-help'>Use your company email and password to sign in</div>
  </form>
);

const MockWorkOrderCard = ({
  workOrder = {
    foNumber: 'WO-001',
  ...
  <article role='article' aria-label={`Work order ${workOrder.foNumber}`} tabIndex={0}>
    ...
    <button type='button' aria-label={`View details for work order ${workOrder.foNumber}`}>
      View Details
    </button>
  </article>
);
  ...
  describe('Dashboard', () => {
    ...
    it('should have accessible metrics', () => {
      ...
      const metrics = container.querySelectorAll(
        '[aria-label*="work orders"], [aria-label*="equipment"]'
      );
